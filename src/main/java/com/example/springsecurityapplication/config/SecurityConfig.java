...
//                .anyRequest().authenticated()
                ...
                .formLogin().loginPage("/product") //страница по умолчанию //////// указываем какой url запрос будет отправлятся при заходе на защищенные страницы
                ...
                .defaultSuccessUrl("/personaccount", true) // Указываем на какой url необходимо направить пользователя после успешной аутентификации. Вторым аргументом указывается true чтобы перенаправление шло в любом случае послу успешной аутентификации
                ...
                .and()
                //Исключение для запроса выхода из csrf
                .csrf().ignoringRequestMatchers(patterns);
        return http.build();
    }
